...
**Recipe content: CC BY-SA 4.0, one platform-wide default, no per-recipe picker.** Deliberately not a free choice: if authors could pick arbitrarily, a proposal merging a CC-BY-NC recipe into a CC0 one would have no correct answer for what license the merge carries. One default sidesteps that. It's already the de facto value in the fixture corpus (`tartine-country-loaf.md`, `mushroom-bourguignon.md`, `mushroom-walnut-ragu.md` all set it), so this mostly ratifies existing practice rather than changing it.

- The default is asserted by the Terms, not written into every recipe's frontmatter. `license:` stays free-text and optional in the schema — it remains the override for content that *isn't* under the platform default (an imported recipe carrying its source page's own terms, say), not a picker UI. `RecipeView.tsx:301-309` currently only renders the license line when `frontmatter.license` is set; it needs a small change to fall back to "CC BY-SA 4.0" when the recipe is public and the field is empty, rather than showing nothing.
- **A third thing the terms have to state, independent of both licenses above:** publishing a recipe as public grants other users the right to fork, edit, and propose changes back *within the app*, regardless of the declared content license — the same way GitHub's own ToS lets anyone fork a public repo without that being a claim about the code's license. Without this clause, the fork/proposal/merge mechanic that is the entire product has no stated legal basis.
